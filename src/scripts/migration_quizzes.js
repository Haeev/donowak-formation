...
function showSqlScript() {
  try {
    const sqlPath = path.join(__dirname, 'create_quizzes_table.sql');
...
async function main() {
  try {
    console.log('=== Migration pour ajouter la table quizzes ===');
    ...
    const apiUrl = `${deploymentUrl}/api/migrations/add-quizzes-table`;
    ...
    try {
      ...
      if (response.ok && result.success) {
        console.log('\n✅ Migration réussie ! La table quizzes a été créée.');
      } else {
