-- Migration complète pour les exercices interactifs
...
DO $$
BEGIN
    -- Vérifier et supprimer les politiques existantes
    IF EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'exercise_attempts' 
        AND policyname = 'Les utilisateurs peuvent voir leurs propres tentatives d''exercices'
    ) THEN
        DROP POLICY "Les utilisateurs peuvent voir leurs propres tentatives d'exercices" ON public.exercise_attempts;
    END IF;
    
    IF EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'exercise_attempts' 
        AND policyname = 'Les utilisateurs peuvent créer leurs propres tentatives d''exercices'
    ) THEN
        DROP POLICY "Les utilisateurs peuvent créer leurs propres tentatives d'exercices" ON public.exercise_attempts;
    END IF;
    
    IF EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'exercise_attempts' 
        AND policyname = 'Les utilisateurs peuvent mettre à jour leurs propres tentatives d''exercices'
    ) THEN
        DROP POLICY "Les utilisateurs peuvent mettre à jour leurs propres tentatives d'exercices" ON public.exercise_attempts;
    END IF;
    
    IF EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'exercise_attempts' 
        AND policyname = 'Les administrateurs peuvent tout voir'
    ) THEN
        DROP POLICY "Les administrateurs peuvent tout voir" ON public.exercise_attempts;
    END IF;
END
$$;
